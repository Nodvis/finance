...
CREATE UNIQUE INDEX "statement_import_rows_account_auth_imported_idx" ON "finance"."statement_import_rows" USING btree ("household_id","account_id","source_namespace",coalesce("source_account_id", "account_id"::text),"authoritative_id") WHERE "status" = 'imported' and "authoritative_id" is not null;--> statement-breakpoint
...
CREATE UNIQUE INDEX "transactions_household_account_authoritative_idx" ON "finance"."transactions" USING btree ("household_id","account_id","source_namespace",coalesce("source_account_id", "account_id"::text),"authoritative_id") WHERE "authoritative_id" is not null;