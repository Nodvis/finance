...
- **Runtime:** the Finance image runs as a non-root user. The default Compose keeps PostgreSQL internal to the Compose network and exposes only Finance on port 3000. Optional Docker hardening is documented separately in `docs/security/deployment-security.md`.
