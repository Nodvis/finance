...
vi.mock("@/lib/categories/service", () => ({
  ...
  archiveHouseholdCategoryEntry: vi.fn(),
}));
...
describe("/api/households/[householdId]/categories/[categoryId]/archive", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("returns 401 when unauthenticated", async () => {
    ...
    const res = await POST(new Request("http://localhost/archive"), {
  ...
  it("returns 404 when category not found", async () => {
    ...
    vi.mocked(archiveHouseholdCategoryEntry).mockRejectedValueOnce(
  ...
  it("returns 200 with archived category on success", async () => {
    vi.mocked(requireHouseholdAccess).mockResolvedValueOnce(testAccess as any);
    vi.mocked(archiveHouseholdCategoryEntry).mockResolvedValueOnce({
      id: validCategory,
      householdId: validHousehold,
      name: "Groceries",
      applicability: "expense",
      archivedAt: new Date("2026-09-08T12:00:00Z"),
    ...
    expect(json.data.archivedAt).toBe("2026-09-08T12:00:00.000Z");
  });
});
