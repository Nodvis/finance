...
import { archiveHouseholdLiabilityEntry } from "@/lib/liabilities/service";
import { archiveLiabilitySchema } from "@/lib/liabilities/schema";
...
export async function POST(
  ...
  try {
    ...
    const parsed = archiveLiabilitySchema.parse(rawBody);
    const archived = await archiveHouseholdLiabilityEntry(
      access,
      liabilityId,
      parsed,
    );
    ...
    return NextResponse.json(
      { data: serializeLiability(archived) },
      { status: 200 },
    );
  } catch (error) {
