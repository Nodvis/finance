...
import { uncompleteSavingsGoalSchema } from "@/lib/savings-goals/schema";
import { handleSavingsGoalRouteError } from "@/lib/savings-goals/error-handler";
import { uncompleteHouseholdSavingsGoal } from "@/lib/savings-goals/service";
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const input = uncompleteSavingsGoalSchema.parse(json);

    const updated = await uncompleteHouseholdSavingsGoal(auth, goalId, input);
