...
import { completeSavingsGoalSchema } from "@/lib/savings-goals/schema";
import { handleSavingsGoalRouteError } from "@/lib/savings-goals/error-handler";
import { completeHouseholdSavingsGoal } from "@/lib/savings-goals/service";
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const input = completeSavingsGoalSchema.parse(json);

    const updated = await completeHouseholdSavingsGoal(auth, goalId, input);
