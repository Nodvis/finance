...
function Table({ title, rows, empty }: { title: string; rows: string[][]; empty: string }) { return <div><h3 className="mb-2 font-medium">{title}</h3>{rows.length ? <ul className="space-y-2 text-sm">{rows.map(([name, value], index) => <li key={`${name}-${index}`} className="flex justify-between gap-3 border-b pb-1 last:border-0"><span className="truncate">{name}</span><span className="whitespace-nowrap">{value}</span></li>)}</ul> : <p className="text-sm text-slate-500">{empty}</p>}</div>; }
