  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(280px,0.65fr)]">
      ...
      <section className="finance-card p-5" aria-labelledby="spending-chart-title">
        ...
        {categoryGroups.length ? <div className="mt-4 grid gap-5" role="group" aria-label={labels.spendingBreakdown}>{categoryGroups.map(([currency, entries]) => { const pieData = entries.slice(0, 6).map((item) => ({ name: item.categoryName ?? labels.uncategorized, value: Math.max(item.percentage, 0) })); return <div key={currency} className="min-w-0"><p className="mb-2 text-xs font-semibold text-[var(--muted-foreground)]">{currency}</p><div className="h-44" role="img" aria-label={`${labels.spendingBreakdown} ${currency}: ${pieData.map((item) => `${item.name} ${item.value}%`).join(", ")}`}><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={pieData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={48} outerRadius={72} paddingAngle={2} stroke="none">{pieData.map((item, index) => <Cell key={`${currency}-${item.name}-${index}`} fill={colorFor(index)} />)}</Pie><Tooltip formatter={(value) => [`${value}%`, labels.spending]} /></PieChart></ResponsiveContainer></div><ul className="space-y-2" aria-label={`${labels.spendingDescription} ${currency}`}>{pieData.map((item, index) => <li key={`${currency}-${item.name}-${index}`} className="flex items-center justify-between gap-3 text-xs"><span className="flex min-w-0 items-center gap-2 truncate text-[var(--muted-foreground)]"><i className="h-2 w-2 shrink-0 rounded-full" style={{ backgroundColor: colorFor(index) }} />{item.name}</span><span className="font-mono font-medium text-[var(--foreground)]">{item.value.toFixed(1)}%</span></li>)}</ul></div>; })}</div> : <div className="mt-5"><EmptyChart label={labels.empty} /></div>}
      </section>
    </div>
  );
}
