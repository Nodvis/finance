...
5. Set the Finance image to `ghcr.io/nodvis/finance:0.1.2`.
6. Run `docker compose up -d`.
7. Open Finance on port 3990 and verify a known household, account and transaction.

The v0.1.2 Finance container listens on port 3990, waits for PostgreSQL, runs pending migrations once through Drizzle's migration journal, and starts the web process only after migration succeeds. Existing data remains in `nodvis-finance-data`; users previously accessing port 3000 must use port 3990 after this update.
