...
- P2 checkpoint: PR #17 merged as `66afdce50f2a55261534fb97eea43b08cc7abf61`; CI initially failed because migration journal entry 0008 was missing, then passed after commit `7044753` (490 unit/integration tests plus 7 Chromium E2E in CI). Migration SQL 0008 passed a private transaction dry-run and was not committed; `drizzle-kit migrate` on the private host still exits with `undefined` before recording 0008, so P2 is not deployed and the private DB has no P2 tables. Backup `/home/erza_agent/backups/nodvis-finance-private-20260908T202707Z-pre-p2.dump` is verified as a PostgreSQL archive with 153 entries.
